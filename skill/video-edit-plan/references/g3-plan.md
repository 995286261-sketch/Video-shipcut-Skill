...
在候选文本索引完成后、进入最终时间线前，按每个语义节拍统计已验证可用镜头的非重复时长。若目标主体镜头不足，或某个外观、模式、武器、动作或人物反应节拍没有直接画面证据，禁止以无关剧情、敌机、环境镜头、冻结画面、循环和无意义慢放来填充机体科普口播。处理分两条路径：

- **已预授权缩短**：`G3-目标主体确认-v<version>.json` 含完整 `sufficiencyFallback`（`preApproved: "shorten_output"`、`approvedBy`、`approvedAt`）时，Agent 可自动采用"缩短成品"，起草短版并直接进入 G2 短版流程；预授权不免除 G3 最终八列回显与 `确认 G3`。
- **未预授权**：G3 必须停在 `review_required`，向用户说明缺口并给出三项选择：补充已授权素材后回到 G0 校验；在 G2 缩短口播并重新批准；或由 G2 改写为当前画面可直接承载的叙事。

短版确定后（无论来自预授权还是用户选择），旧长版口播和其音频时间码进入 `supersededDraftRefs`；G2 必须生成新的获准稿、对应完整旁白及同源时间码。G3 的时长、BGM 入出点、节拍表、镜头验证和最终回显均从短版输入重新生成，不能沿用旧长版时间槽。
...
多模态视觉适配器通常能识别"有机体出现"，区分具体机体类型（如独角兽高达 vs 刹帝利 vs 新安州）则依赖明确的识别与排除规则。因此身份确认是**政策级、一次性**的，不是逐片段的：

1. 用户在 `工作台/<projectId>/G3-剪辑计划/G3-目标主体确认-v<version>.json` 中确认一次：`targetSubject.userConfirmed` 确认的是主体身份 `canonicalName`、`identificationRules` 和 `exclusionRules`；不要求用户逐片段指出"哪一段有目标机体"。
2. 此后 Agent 按该规则对每个候选逐帧判定 `identityStatus`，证据只能是实际抽取帧与视觉观察账本记录；`uncertain`、`mixed`、`person_only`、`not_present` 一律不自动入选，只进入不可用素材统计。
3. 校验计划时传入 `--subject-confirmation <G3-目标主体确认.json>`，校验器检查其 projectId、节点、`userConfirmed: true`、识别与排除规则非空；`approved_for_g4` 放行时 `timelineReview.basisRefs` 必须引用该确认文件。
4. 人工唯一审批仍是最终 G3 八列回显与精确确认串 `确认 G3`；身份判定本身不设逐片段人工门禁。
5. 如果素材中目标机体戏份不足，按"素材充分性与短版分流"处理：自动缩短成品仅在确认文件含完整 `sufficiencyFallback` 预授权时允许，否则停下呈现三项选择。
...
1. **探索层：** 镜头检测优先——先对全片跑 `ffmpeg scdet`，每个检出的子镜头至少抽取中帧，用户确认的固定间隔网格只用于补足检测盲区并控制发现成本，记录检测器阈值、切点数、子镜头帧数、网格间隔、模型、提示词和缓存键。快切素材上固定网格会系统性漏短镜头（N8 ㉓ 活测：14 分钟快切 AMV，581 个子镜头 vs 网格仅采到其一角，scdet 即使提到 0.35 阈值仍偏保守）。间隔与阈值只决定候选发现成本，不能用来放行最终镜头。
