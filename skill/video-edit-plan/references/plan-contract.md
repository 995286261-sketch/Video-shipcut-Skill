...
转场决定（002 挂空合同修复批，2026-09-21）：每段可带 `transitionInstruction`（**缺省=硬切**）与 `transitionDurationMs`。词表 `硬切 / 叠化 / 黑场入 / 黑场出 /（抹开=预留档，第一批禁用）`——**单一指令逐段批准，"叠化/硬切按 G4 微调"式含糊句从词表层面进不了卡**。语义：`叠化` 是本段退场进入下一段的交叠指令（不得在末段），`黑场入/出` 仅允许在首段入场/末段退场（时间内渐亮渐暗，不吃素材余量）。模型规则——转场窗口只准落在口播停顿内（成片网格不变、音不动画面服从）、手柄余量双向 ≥D/2（不足即摊牌，由用户逐切点选降硬切/挪切点/缩时长，G4 永不静默降级）——唯一事实源在 `skill/transition-expert/references/transition-contract.md`，由 `transition_validate_plan.py` 深检；节点侧 validator 只查词表、时长在场与回显-计划逐字一致。
