...
G1 继承 G0 已收集的信息（受众、平台、风格、BGM），只补充 G0 没问的。标题与叙事角度按 `g1-choice-cards.md` 生成候选并**预选推荐项**（单卡模式，2026-09-11 用户定案：不独立停靠提问），随后整理事实主张与表达边界；全部决策点连同【默认=推荐｜依据】并入 G1 最终回显，一次呈报。

同一主题内允许结构化子项。用户否决某条时才重新展开该条的候选往来。
...
1. **参考视频分析**（如果有）：
   ...
   **(e) 音轨分析（BGM 专员接口，零视觉 Token）**：把参考视频文件本身交给 music-expert 工具分析音轨并派生风格简报：
   ```bash
   python skill/music-expert/scripts/music_analyze.py --input <参考视频> \
     --output-dir 工作台/<projectId>/G1-创作方向/G1-参考视频分析/ \
     --style-brief-out 工作台/<projectId>/G1-创作方向/G1-参考视频分析/BGM-风格简报-<短名>-v0.1.json
   ```
   - 产物《BGM-分析报告》《BGM-分析回显》《BGM-风格简报》与视觉分析产物同目录存放，一并由 manifest 引用；不得写入素材包或原始参考视频目录。
   - 简报是**纯机器事实**（BPM、bpmRange、能量形状、分段结构），不含"建议曲目"；`suggestedQueryTerms` 由脚本留空，Agent 依本片题材/情绪在回显中补写自由检索词（供后续找乐环节使用，用户未要求找乐时不补写）。
   - 同一视频同版本重跑命中 `cache_hit`，产物字节相同搬迁到请求目录，禁止重复全片分析；缺分析运行时按工具返回的 blocked 原样上报，不得用"听感印象"冒充实测数字。

   **(f) 产物保存**：
   - 只在项目工作台保存唯一 `reference-analysis-manifest-v<version>.json`，并由它引用结构化报告、场景时间码、关键帧、转场拼接图和音轨分析报告/风格简报。
   - 技术报告、关键帧和拼接图均为 manifest 的派生产物，全部留在 `工作台/<projectId>/G1-创作方向/G1-参考视频分析/`。不得向源片目录、全局目录或归档目录双写缓存。

   回显时展示报告路径、场景数量、转场类型分布和关键文字样式摘要；有音轨时**内联展示《BGM-分析回显》卡的完整内容**（头部简述+逐段表格，不是只给文件链接），说明它是 G3 BGM 节奏规划与 G4 铺轨的机器输入。回显固化原则：凡节点接了专员，专员的回显卡必须在节点回显里原样可见。
2. **标题**：基于素材包和 G0 摘要给 2-3 个候选，每个候选标注 `editorial_expression`、`factual_claim` 或 `mixed`；Agent 预选其中推荐项为默认。
3. **叙事角度**：根据项目类型生成 3 个推荐角度（+用户可否决后自定义）；Agent 预选推荐角度为默认。
