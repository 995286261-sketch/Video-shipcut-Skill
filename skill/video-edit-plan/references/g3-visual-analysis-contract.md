...
缓存键目前只扫本项目工作台与 `--cache-root`；同 SHA 素材在别的项目已有全片分析时，允许**人工搬迁**（manifest、帧图、账本原样拷入本项目 G3 目录），不视为重跑。搬迁件必须：manifest 顶层追加 `provenance` 注记，写明源项目、原 manifest 路径与搬迁日期；观察账本记录保持原 `createdAt`，不得伪装成本项目新分析。目标主体确认是**项目级**判断，搬迁只复用"同一帧图上看见了什么"的客观记录，绝不复用原项目的人物认定结论；源项目报告若 provider/model 字段缺失（旧合同时期产物），如实保留 legacy 标注，不得补写猜测。正式的同 SHA 跨工作台缓存通道是否建立（与 music_analyze 的 cacheKey 搬迁行为对齐）仍待裁决，本条为裁决前的默认路径。
