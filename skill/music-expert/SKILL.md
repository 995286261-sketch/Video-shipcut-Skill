...
- `scripts/music_analyze.py`、`scripts/music_search_terms.py`、`scripts/music_search_freesound.py`、`scripts/music_search_netease.py`、`scripts/music_register_candidate.py`、`scripts/music_recommend.py`、`scripts/music_align.py`、`scripts/music_mix_plan.py`：八个唯一入口（`music_tags.py` 共享词表、`music_echo.py` 固定表格回显卡渲染模块）。
