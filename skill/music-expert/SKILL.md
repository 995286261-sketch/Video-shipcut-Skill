...
2.6 检索词怎么来 → 按 `references/search-terms-contract.md`：**Agent 推导（口头偏好＞主题翻译＞风格简报锚定，简报可选；口播稿不做主源），脚本不编造曲库命中** → `music_search_terms.py --term <词>… --note "词=依据"…`（可选 `--preference/--theme/--style-brief/--timeline-ms`）出《BGM-检索词-v0.1》+回显卡：逐词标来源与推导依据、时长下限由成片时长推导、bpmRange 取自简报；**单卡模式（2026-09-11 定案）：卡不独立停靠呈报，随依据并入节点最终回显，默认词直接进检索，用户否决词才重跑**；词卡不设门禁口令。体裁标签红线：没被耳朵（人或模型）听过的曲风描述必须标"未试听推测"；**参考曲在场时出词前必跑参照曲画像（见 2.7 `--reference-note`），词以画像原话为准——003 问题④实锤：跳过此步两轮 87 首白跑**。
...
2.7 锚定打分排出短名单后，加"模型试听笔记"（`references/listen-notes-contract.md`；**做默认选曲时 A/B 笔记是必备输入不是可选项**）→ A/B 贴合参考：`music_listen_omni.py --reference <用户确认的参照曲> --manifest <推荐或候选json>`；绝对属性入库：`--absolute --library experience/music`（听过的歌零成本复用库内同模型同提示词笔记，只有没听过的才调用，听完自动写回身份卡+笔记）。**参照曲画像（检索词之前的耳朵证据，003 问题④/⑤）：`--reference <参照曲> --reference-note`（不给 --manifest）单出《参照曲风格画像》卡**——长参照曲取 2–3 段节选合听，防单一静段冒充全曲风格，产物含可直取的英文检索词，是 2.6 词卡体裁红线的一等入口。A/B 重排名按（参照曲SHA×候选SHA×提示版本×模型×用途）在**项目内**缓存复用（`试听AB-缓存.json`，不进经验库，A/B 相对评价永不作库事实，003 问题⑥）。先探测听觉能力，**没有能力就明说并且一个字都不写**，调用失败如实进 `partialFailures`。A/B 笔记相对参照曲、不入库；绝对笔记入 `listen.md`。笔记只是参考，不做门禁；只听短名单（`--max-tracks` 封顶）控制计费。**默认选曲规则（单卡模式）：默认项=判"能使用"中的最高适配，纯机器分第一不得当推荐；capability_missing 时不生成默认项、退回挑曲停靠点问用户**（主线 operator-dialogue 单卡合同，N8 活测证据）。
