...
条数 2–6，多了去重截断；每张卡必须逐词写明 `rationale`（哪条推导、依据什么）。**单卡模式（用户 2026-09-11 定案）：词卡不独立停靠呈报**——默认词表直接进检索，卡随依据并入节点最终回显作证据产物，用户否决词才重跑；检索词卡不设门禁口令，挑曲停靠点仅在**听觉能力缺失**时复活（有耳朵时默认选曲=模型判"能使用"中的最高适配项，见 g1-choice 与主线单卡合同）。**体裁标签红线**：给没被任何耳朵（人或模型）听过的曲子写风格词，必须显式标"未试听推测"，能先跑绝对笔记的先跑笔记，词以笔记原话为准（N8 活测：Agent 脑补"管弦"、实测为工业 techno，整轮检索报废）。
...
2. G1 末（方向简报成形、G1 最终确认回显**之前**——2026-09-11 用户定案：找乐结果与方向同呈一张确认卡）Agent 按本合同推导词、调 `music_search_terms.py` 出卡；
3. 检索→推荐→模型试听出默认选曲（能力缺失则停下问用户）→整轨登记回填。分析时点随"初次分析统一进 G1"的节点侧定案走（见交接文档）。
