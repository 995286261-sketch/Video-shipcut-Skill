...
- **时长口径两档（002 问题③裁决）**：时长下限先作硬过滤；若因此零候选且下限高于默认 15s，自动做**一次**放宽重试（`durationFilter.mode = "soft-fallback"`），短于目标的候选保留并逐条标注 `durationShortfall`（需求下限/实际时长）。G1 卡片必须把"循环铺满 / 拼接多段 / 放宽画像 / 无 BGM"作为显式决策呈出——短候选进卡是透明的，绝不静默凑时长。
...
- **G1**：参考视频音轨分析产出风格简报（N3，已接线）。**找乐检索通道路由（用户 2026-09-17 定案）**：默认按项目分发边界选轨——对外（`public_*`/商业）只自动检索轨道一 Freesound（真实许可证据链）；个人/内测默认可用轨道三 网易云（未清权标记不变）；通道本身在 G1 回显卡上是**可切换决策点**（默认=按边界），未清权候选在对外项目仅可作风格参照列席，成片资格红线不随通道切换松动。编排细则见 video-edit-plan SKILL《G1 末：找乐编排》第 3 步。**G2**：节拍蓝图创作循环（N9）。**G3**：`music_align` 对齐产物入计划 bgmPlan 哈希链（N5）。**G4**：`music_mix_plan` 合同 + `g4_assemble` 逐字执行（N6）。**G5**：`g5_audit_bgm_chain` 消费装配记录 bgmMix 全链对账（N7）。
