...
- `transition-audit.json`: required handshake from transition-expert — run `skill/transition-expert/scripts/transition_report.py --manifest <G4工程> --assembly-record <装配记录> --directive <转场执行指令>` and file the report in the bundle. G5 validates identity, `status passed`, `gridInvariant`, and that the recorded master hash matches the shipped `final-video.mp4` (stale = re-render without re-audit). Projects with no transitions still file it (`transitions: []`, passed). The audit proves filter==approval only; mid-transition check frames stay a human visual duty (⑧).
