...
> 本项目原名 Video-shipcut，v1.1.0 起按公司平台项目代号统一更名为 **P0-C**，编排入口 Skill 为 `p0-c-pipeline`。当前版本 **v1.6.0（试装预览版本）**：转场试装预览四批落地——批准前每个非硬切切点产低清无声小样（与成片代码同源防幻觉、窗口被批准裁切钳制、源 sha 三方对账），G3 硬门禁（缺预览=卡校验拒并点名切点、改版即 stale、唯一豁免=如实披露"你批准的是未见过的效果"），终审卡表后统一附「▶ 一页看全部」观看页（脚本生成、代码即规格），sinjuku 真源活体彩排+用户视角验收通过。上一里程碑 v1.5.0（转场专员版本）：transition-expert 立册、三定律入合同、首次全链实片验收闭环（验收003）与 29 条测后修复零开放。
