...
> 本项目原名 Video-shipcut，v1.1.0 起按公司平台项目代号统一更名为 **P0-C**，编排入口 Skill 为 `p0-c-pipeline`。当前版本 **v1.5.0（转场专员版本）**：transition-expert 立册并首次全链实片验收闭环（验收003，sinjuku-intro-001 三处叠化实片、六门禁全过、交付包 valid），"音不动画面服从、手柄回填网格不变、缺料摊牌永不静默降级"三定律入合同，同批收口验收003 问题清单 29 条测后修复（零开放，含口令归一化自然句形、连续网格机器强制、长链转场时基盲区补例）。上一里程碑 v1.4.0（字幕专员版本）：subtitle-expert 剥出立册与彻底化，"底座+插件"架构定案。
