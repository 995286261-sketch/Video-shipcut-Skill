...
- **瑕疵2**：G0/G2 卡片示例残留旧路径 `工作区/素材包/<projectId>/…`。**状态：已验证（2026-09-08）**；g0-start-form、g2-choice-cards、path-display-contract 正反例、g3-plan 抽帧输出示例、g1-direction-guide 与 video-edit-plan SKILL 的 G1 输出路径全部统一为 `工作台/<projectId>/G0-素材包/…` 布局；`工作区/` 仅保留"历史项目只读兼容"表述。验证：`grep -r "工作区/素材包" skill/` 零命中；全量回归通过。修复提交：待本批次验收后创建。
...
- **⑨**：`g1_direction.py write` 写入旧路径，违反项目布局合同。**状态：已验证（2026-09-08）**；`target_directory` 改为 `<workspace>/工作台/<projectId>/G1-创作方向/`，`--on-conflict version` 生成 `G1-创作方向-v<N>` 兄弟目录；新正向回归钉住布局路径。验证：`~/.local/bin/python3 skill/video-edit-plan/tests/test_g1_direction.py`（12/12 通过，含写入落点与版本化冲突）。修复提交：待本批次验收后创建。
...
- **㉙**：`g4_prepare.py` 固定写 `G4-可编辑工程-v0.2.json`，重跑会覆盖上一批次清单。**状态：已验证（2026-09-08）**；清单文件名版本从计划文件名（如 `G3-剪辑计划-v0.2.json` → `G4-可编辑工程-v0.2.json`）派生，目标清单已存在时拒绝写入，除非显式 `--force`。验证：`~/.local/bin/python3 skill/local-video-render/tests/test_g4_contract.py`（6/6 通过，含重跑"already exists"拒绝与 `--force` 放行）。修复提交：待本批次验收后创建。
- **㉚**：`g4_render.py --output-dir` 直接平铺切片，与合同约定的 `clean-segments/` 子目录语义不一致。**状态：已验证（2026-09-08）**；切片固定输出到 `<output-dir>/clean-segments/` 并在结果 JSON 中回显 `segmentsDir`；每段 ffmpeg 退出后按 ㉒ 规则核验产物存在且非空，"退出 0 但未落盘"即失败。验证：`~/.local/bin/python3 skill/local-video-render/tests/test_g4_render_profile.py`（2/2 通过，含 `clean-segments/seg-001.mp4` 存在与平铺路径不存在断言）。修复提交：待本批次验收后创建。
