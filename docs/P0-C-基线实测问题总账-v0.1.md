...
- **瑕疵1**：v1.1 质量字段进入 SKILL.md，但没有同步进入固定 G3/G5 卡片模板；人工审批可能对着旧清单。**状态：已验证（2026-09-08）**；G3 门禁收据新增必填项 `packaging_decisions`（八列表后固定"成片包装决定"一节：画幅策略、封面帧与时长、章节卡插入点、句子级口播边界、字幕样式含章节卡隐藏规则、BGM ducking、clean master 确认），G5 新增 `check_frames`（封面检查帧+字幕检查帧为证据文件）；两张选择卡模板同步补字段。验证：`~/.local/bin/python3 skill/p0-c-pipeline/tests/test_pipeline_state.py`（7/7 通过，收据缺新项即拒绝）。修复提交：`5e291d3`（Enforce workbench layout, audio decode probe, and G4 manifest hygiene）。
- **瑕疵2**：G0/G2 卡片示例残留旧路径 `工作区/素材包/<projectId>/…`。**状态：已验证（2026-09-08）**；g0-start-form、g2-choice-cards、path-display-contract 正反例、g3-plan 抽帧输出示例、g1-direction-guide 与 video-edit-plan SKILL 的 G1 输出路径全部统一为 `工作台/<projectId>/G0-素材包/…` 布局；`工作区/` 仅保留"历史项目只读兼容"表述。验证：`grep -r "工作区/素材包" skill/` 零命中；全量回归通过。修复提交：`5e291d3`（Enforce workbench layout, audio decode probe, and G4 manifest hygiene）。
...
- **⑨**：`g1_direction.py write` 写入旧路径，违反项目布局合同。**状态：已验证（2026-09-08）**；`target_directory` 改为 `<workspace>/工作台/<projectId>/G1-创作方向/`，`--on-conflict version` 生成 `G1-创作方向-v<N>` 兄弟目录；新正向回归钉住布局路径。验证：`~/.local/bin/python3 skill/video-edit-plan/tests/test_g1_direction.py`（12/12 通过，含写入落点与版本化冲突）。修复提交：`5e291d3`（Enforce workbench layout, audio decode probe, and G4 manifest hygiene）。
- **⑩**：源素材 640×360@23.98fps，低清 warning 正常触发，不是脚本 bug。
- **⑪**：macOS `say` 音色缺失时静默回退英文，中文几乎不读；需 ASR 回环自检。**状态：已验证（2026-09-08）**；新增 G2 `local_tts.py` 统一配音合成入口：合成前用 `say -v ?` 预检声线，未安装即 `missing_voice` 结构化 blocked，macOS 静默回退被机器拦截；可选 `--verify-asr` 用受控 Faster-Whisper 运行逐句回读，归一化文本相似度低于阈值即阻断，`--asr-initial-prompt` 传入专名词表防止 ASR 误听专有名被误归因于音色。验证：`~/.local/bin/python3 skill/media-evidence-prep/tests/test_local_tts.py`（3/3 通过；含未安装音色阻断，及受控运行时下婷婷合成→whisper 回读相似度 ≥0.6 的端到端用例）。修复提交：`ae49915`（Script voice synthesis, whisper alignment, and keyframe cache reuse）。
- **⑫**：原始设计没有 BGM 内容/节奏分析能力；这是新增 Skill 设计，不是代码遗失。**状态：部分推进（2026-09-08）**；独立 `music-expert` Skill 已落地确定性分析引擎（见⑯）与**双轨找乐**：自动轨道 `music_search_freesound.py`（Freesound 官方 API，CC0/CC-BY 过滤 + 每候选授权证据链 + 解码探针，缺 token 结构化 blocked 不静默换源）、人工轨道 `music_register_candidate.py`（Pixabay/Mixkit 无 API 曲库由人合法下载后登记许可证据）；`music_recommend.py` 按画像打分并把候选不足显式上报不硬凑。**尚未闭环，保持未结**：① 自动轨道需 `P0C_FREESOUND_TOKEN` 做一次真实联网检索取证（用户注册后）；② 同事产品化"AI 自己找 BGM"的运行证据（配置字段/搜索调用/授权链）仍未取得，不据此判 ⑫ 已解决；③ 六节点接线（G0 `use_library_later` 调本 Skill、G3 引用卡点表、G5 审计许可）另起批次，本版不改管线。分析侧实现见 `efebb4e`。
- **⑬**：G2 缺依赖时结构化 blocked，行为正确，作为正面控制项。
- **⑭**：Faster-Whisper 路径、模型目录与 CLI 参数没有被脚本化声明，运行依赖靠环境变量 workaround。**状态：已验证（2026-09-08）**；`g3_align_narration_audio.py` 按 `local_transcribe.py` 的受控运行时模式重写：`P0C_FASTER_WHISPER_HOME` 加载运行时包，新增 `--model-dir`（缺省 `P0C_FASTER_WHISPER_MODEL_HOME`）与 `--language`，只用本地缓存快照；缺目录/缺快照/缺依赖均返回结构化 `blocked` 退出码 2（不再顶层 import、不再 traceback、禁止下载），同音频+模型+语言按 `cacheKey` 直接 `reused`。验证：`~/.local/bin/python3 skill/video-edit-plan/tests/test_g3_align_narration_audio.py`（4/4 通过，三类 blocked 均为可解析 JSON、无 traceback）。修复提交：`ae49915`（Script voice synthesis, whisper alignment, and keyframe cache reuse）。
- **⑮**：macOS `say` 直接输出 wav 的格式兼容问题，需要 CAF→FFmpeg 转换 workaround。**状态：已验证（2026-09-08）**；workaround 固化进 `local_tts.py`：`say --data-format "LEI16@22050" -o *.caf` 后 FFmpeg 转 22050Hz 单声道 wav，两步都核验产物真实落盘且非空（沿用㉒规则），不再靠调用方临场处理。验证：与⑪同一 `test_local_tts.py`（真实中文句子合成出可探测时长的 wav）。修复提交：`ae49915`（Script voice synthesis, whisper alignment, and keyframe cache reuse）。
- **⑯**：BGM onset/卡点产物没有可运行生成脚本，无法机器复现。**状态：已验证（2026-09-08）**；新建独立 `music-expert` Skill 的 `scripts/music_analyze.py` 作为可运行生成器：librosa 节拍/起音跟踪 + 前缀和贪心变化点能量分段 + ebur128 响度，输出含 `beatsMs/onsetsMs/hitPoints`（卡点表）`energySegments` 的《BGM 分析报告》，按源文件 `cacheKey` 复用、产物核验非空、缺受控运行时结构化 `blocked`。验证：`skill/music-expert/tests/test_music_analyze.py`（13/13 含正向/缓存/阻断）+ 全量回归 19/19 + 真实 163s 参考片活体（4.2s，123BPM、8 段、637 卡点、重跑 cache_hit）。修复提交：`efebb4e`（Add music-expert support skill）。（本项是新增能力落地，六节点接线另见 ⑫。）
- **⑰**：G2 文本估时与实际 TTS 时长偏差约 40%，应先合成再估时。**状态：已验证（2026-09-08）**；`local_tts.py` 输出逐句 `durationMs` 与 `measuredTotalDurationMs` 配音清单（`G2-配音清单-v0.1.json`）；合同改为：旁白已合成时 `narrationEstimatedDurationSec` 必须取实测值，未合成才可文本估计并须标注，G2 声音卡试听记录强制附实测总时长。验证：`test_local_tts.py` 断言总时长=逐句之和且逐句>100ms。修复提交：`ae49915`（Script voice synthesis, whisper alignment, and keyframe cache reuse）。
...
- **㉓**：G0 音频 register 只哈希不解码探针，损坏网易云加密文件一路流到 G3 才暴露。**状态：已验证（2026-09-08）**；`material_pack.py register` 对每个音频条目跑 ffmpeg 完整解码探针（`-f null`），探针通过才写 `decodeProbe: passed`；任一音频解码失败（含 ffmpeg 缺失时有音频在场）即返回 `blocked` + `audio_decode_probe_failed` 并列出逐文件错误，**不写 manifest**，损坏文件在 G0 即被拦下。验证：`~/.local/bin/python3 skill/material-pack-intake/tests/test_material_pack.py`（10/10 通过，含伪造 `CTENFDAM` 头 mp3 阻断且无 manifest、真实 sine wav 探针通过）。修复提交：`5e291d3`（Enforce workbench layout, audio decode probe, and G4 manifest hygiene）。
...
- **㉕**：`g4_prepare.py` 不读取 `durationDecision`，产出 `targetDurationMs:0`。**状态：已验证（2026-09-08）**；`g4_prepare.py` 以 `plan.durationDecision.targetDurationSec` 为第一优先来源（写入 `durationDecisionRef` 追溯），仅其缺失时回落 targetProfile；两处皆无正值时拒绝准备（"G4 refuses to prepare with a zero target"），清单零目标时长被机器阻断。验证：`~/.local/bin/python3 skill/local-video-render/tests/test_g4_contract.py`（6/6 通过，含 `durationDeltaMs==0` 与零目标负向阻断）。修复提交：`5e291d3`（Enforce workbench layout, audio decode probe, and G4 manifest hygiene）。
...
- **㉘**：人工 QA 否决系统 TTS 的人机感；G2 没显式标注系统 TTS 仅预览级，也没有在声音卡上强制神经 TTS/真人替代路径。**状态：已验证（2026-09-08）**；声音卡每个选项新增强制"成片分级"列：神经 TTS（如已授权百炼 CosyVoice）/真人可为成品级，macOS 系统声音一律预览级且必须注明人机感与升级路径，未告知分级不得让用户按"能听"通过；`local_tts.py` 清单固定输出 `voiceTier: preview_only` 与分级说明，试听确认字段加入分级与实测总时长；G2 SKILL 与选择卡合同同步。修复提交：`ae49915`（Script voice synthesis, whisper alignment, and keyframe cache reuse）。
- **㉙**：`g4_prepare.py` 固定写 `G4-可编辑工程-v0.2.json`，重跑会覆盖上一批次清单。**状态：已验证（2026-09-08）**；清单文件名版本从计划文件名（如 `G3-剪辑计划-v0.2.json` → `G4-可编辑工程-v0.2.json`）派生，目标清单已存在时拒绝写入，除非显式 `--force`。验证：`~/.local/bin/python3 skill/local-video-render/tests/test_g4_contract.py`（6/6 通过，含重跑"already exists"拒绝与 `--force` 放行）。修复提交：`5e291d3`（Enforce workbench layout, audio decode probe, and G4 manifest hygiene）。
