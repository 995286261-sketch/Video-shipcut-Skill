...
- **㉒**：末尾抽帧 ffmpeg 退出 0 但不落盘，脚本不检查文件存在。**状态：已验证（2026-09-08）**；两个抽帧脚本（验证帧、关键帧）对每张帧图核验存在且非空，未落盘时以 100ms 步长最多回退 500ms 重试，清单同时记录 `requestedSourceMs` 与实际 `sourceMs`，回退窗口内仍无产物即结构化失败；`g4_assemble.py` 复用同一"产物必须真实存在"规则。验证：`~/.local/bin/python3 skill/video-edit-plan/tests/test_g3_frame_extraction.py`（5/5 通过，真实 ffmpeg 正向落盘、假成功 shim 拒绝、末尾边界回退到 1499ms 均被断言）。修复提交：待本批次验收后创建。
